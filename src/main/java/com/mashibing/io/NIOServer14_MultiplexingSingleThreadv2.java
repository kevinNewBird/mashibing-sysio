...
public class NIOServer14_MultiplexingSingleThreadv2 {
...
//                System.out.println(keys.size()+"   size");
                while (selector.select(50) > 0) {
                    ...
                    while (iter.hasNext()) {
                        ...
                        iter.remove();//移除的是事件有状态的结果集的对象, 而不是kernel内核里的
                        if (key.isAcceptable()) {
                            acceptHandler(key);
...
//            }
    ...
    public static void main(String[] args) {
        NIOServer14_MultiplexingSingleThreadv2 service = new NIOServer14_MultiplexingSingleThreadv2();
        service.start();
    }
}
